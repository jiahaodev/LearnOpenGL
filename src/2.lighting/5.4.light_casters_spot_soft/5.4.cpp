...
int main() {
	...
	//初始化shader程序
	Shader lightingShader("shader/2/5.4.light_casters.vs", "shader/2/5.4.light_casters.fs");
	Shader lampShader("shader/2/5.4.lamp.vs", "shader/2/5.4.lamp.fs");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		lightingShader.setFloat("light.cutOff", glm::cos(glm::radians(12.5f)));//内圆锥
		lightingShader.setFloat("light.outerCutOff", glm::cos(glm::radians(17.5f)));//外圆锥
...
void initShaderData()
{
	...
	//【箱子相关】 first, configure the cube's VAO (and VBO)
	...
	//【光源相关】 second, configure the light's VAO (VBO stays the same; 
