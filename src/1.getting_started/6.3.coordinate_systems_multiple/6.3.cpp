...
int main() {
	...
	//初始化shader程序
	//initShaderProgram();
	Shader ourShader("shader/1/6.3.coordinate_systems.vs", "shader/1/6.3.coordinate_systems.fs");
	//初始化shader相关数据
	initShaderData();
	//初始化纹理数据
	initTextureData();
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		float num =	sin(glfwGetTime()) ; //动态改变观察矩阵
		view = glm::translate(view, glm::vec3(num, 0.0f, -3.0f));
		...
		//ourShader.setMat4("projection", projection); //暂时没有这个接口
		...
		for (unsigned int i = 0;i < 10; i++)
		{
			...
			glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
			glDrawArrays(GL_TRIANGLES, 0, 36); //开始绘制
		}
